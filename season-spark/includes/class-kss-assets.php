...
class KSS_Assets {
    ...
    public static function enqueue_front_assets( $only_if_enabled = true ) {
        ...
        if ( $load_cdn ) {
            // Load tsparticles from local plugin path (WP.org-compliant)
            wp_register_script(
                'kss-tsparticles',
                KSS_PLUGIN_URL . 'assets/vendor/tsparticles/tsparticles.bundle.min.js',
                array(),
                '2.11.0',
                true
            );
        ...
        $images = [
            ...
            // Spark (used for Independence effect)
            'spark'     => $images_dir . 'spark.svg',
            // Cursor (fallback)
            'cursor'    => $images_dir . 'spark.svg',
        ];
    ...
    public static function enqueue_admin_assets( $hook ) {
        // Load on any Season Spark admin page (top-level or subpages)
        if ( false === strpos( $hook, 'kss-settings' ) ) {
            return;
        }
        ...
        // Enqueue media scripts for the WP media modal (used by Custom Graphics fields)
